...
public class Manutencoes extends javax.swing.JFrame {
    ...
    public Manutencoes() {        
        ...
        ManutencaoControle manutencaoes = new ManutencaoControle();
        ClienteControle clientes = new ClienteControle();
        
        DefaultTableModel linha = (DefaultTableModel) this.tabela.getModel();
        
        String[] c = new String[6];
        for(Manutencao manutencao : manutencaoes.getLista()){                    
            c[0] = clientes.retornaNome(manutencao.getCpfCliente().trim());
            c[1] = manutencao.getCpfCliente().trim();
            c[2] = String.format("%.2f", manutencao.calculaPreco());
            c[3] = "" + manutencao.getData().toString().trim(); 
            c[4] = (manutencao.isPago())? "Sim" : "Não";
            c[5] = manutencao.getDesc().trim();
            linha.addRow(c);
        }
    }
    ...
    private void initComponents() {
        ...
        tabela.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] { },
            new String [] {
                "Cliente", "CPF","Valor", "Data", "Pago", "Descrisão"
            }
        ));
    ...
    private void btn_ApagarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ApagarActionPerformed
        if(this.tabela.getSelectedRow() != (-1))
        {
            ...
            manutencaos.apaga(this.tabela.getSelectedRow());
            new Manutencoes().setVisible(true);
            dispose();
        }
    ...
    private void btn_EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_EditarActionPerformed
        if(this.tabela.getSelectedRow() != (-1))
        {
            ...
            new CadastraManutencao(this.tabela.getSelectedRow()).setVisible(true);
            dispose();
        }
    ...
    private void btn_BuscaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_BuscaActionPerformed
        ...
        {            
            ...
            {
                manutencaos.busca( 1, cp_Busca.getText().trim());
            }
            if( this.radioData.isSelected())
            {
                manutencaos.busca( 3, cp_Busca.getText().trim());
            }
        }
        if( this.radioPago.isSelected() )
        {
            manutencaos.busca( 2, "true");            
        }
        if( this.radioNaoPago.isSelected() )        
        {                   
            manutencaos.busca( 2, "false");            
        }        
        new Manutencoes().setVisible(true);
        dispose();        
    ...
    private void btImprimeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btImprimeActionPerformed
        ...
        {            
            ManutencaoControle manutencaos = new ManutencaoControle();
            manutencaos.imprime(this.tabela.getSelectedRow());
        }
