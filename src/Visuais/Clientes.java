...
public class Clientes extends javax.swing.JFrame {
    ...
    public Clientes() {        
        ...
        for(Cliente cliente: clientes.getLista()){
            c[0] = cliente.getNome();
            c[1] = cliente.getCpf();
            c[2] = cliente.getTelefone();
            c[3] = cliente.getEndereco().toString();
            linha.addRow(c);
        }
    }
    ...
    private void btn_ApagarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ApagarActionPerformed
        ...
        {
            ClienteControle clientes = new ClienteControle();
            clientes.apaga(this.tabela.getSelectedRow());
            new Clientes().setVisible(true);
            dispose();
        }
    ...
    private void btn_EditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_EditarActionPerformed
        if(this.tabela.getSelectedRow() != (-1))
        {
            new CadastraCliente(this.tabela.getSelectedRow()).setVisible(true);
            dispose();
        }
    ...
    private void btn_BuscaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_BuscaActionPerformed
        if( cp_Busca.getText().trim() != null && !cp_Busca.getText().trim().equals(""))
        {
            ...
            {
                clientes.busca( 1, cp_Busca.getText().trim());
            }
            if( this.radioCpf.isSelected() )
            {
                clientes.busca( 2, cp_Busca.getText().trim());
            }
            if( this.radioTelefone.isSelected())
            {
                clientes.busca( 3, cp_Busca.getText().trim());
            }
            new Clientes().setVisible(true);
            dispose();
        }
